    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2.5">
          {/* Queued Reports Banner (when offline reports exist) */}
          {unsyncedCount > 0 && (
            <div className="p-2.5 rounded-xl border border-amber-500/40 bg-amber-950/30 text-amber-300 text-xs flex items-center justify-between shadow-sm">
              <div className="flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0" />
                <div>
                  <div className="font-bold text-[11px] text-slate-100">{unsyncedCount} report(s) queued locally</div>
                  <div className="text-[9.5px] text-slate-400">Cached on device &bull; Auto-syncs on Smart Sync</div>
                </div>
              </div>
              <button
                type="button"
